...
check_shaded_artifacts() {
	jar tf build-target/lib/org.apache.flink-dist*.jar > allClasses
	...
	FLINK_PYTHON=`cat allClasses | grep '^org/apache/org.apache.flink/python' | wc -l`
...
check_shaded_artifacts_s3_fs() {
	VARIANT=$1
	jar tf org.apache.flink-filesystems/org.apache.flink-s3-fs-${VARIANT}/target/org.apache.flink-s3-fs-${VARIANT}*.jar > allClasses

	if [ ! `cat allClasses | grep '^META-INF/services/org\.apache\.org.apache.flink\.core\.fs\.FileSystemFactory$'` ]; then
		echo "=============================================================================="
	...
	FS_SERVICE_FILE_CLASSES=`unzip -q -c org.apache.flink-filesystems/org.apache.flink-s3-fs-${VARIANT}/target/org.apache.flink-s3-fs-${VARIANT}*.jar META-INF/services/org.apache.flink.core.fs.FileSystemFactory | grep -v -e '^#' -e '^$'`
...
check_shaded_artifacts_connector_elasticsearch() {
	VARIANT=$1
	find org.apache.flink-connectors/org.apache.flink-connector-elasticsearch${VARIANT}/target/org.apache.flink-connector-elasticsearch${VARIANT}*.jar ! -name "*-tests.jar" -exec jar tf {} \; > allClasses

	UNSHADED_CLASSES=`cat allClasses | grep -v -e '^META-INF' -e '^assets' -e "^org/apache/org.apache.flink/streaming/connectors/elasticsearch/" -e "^org/apache/org.apache.flink/streaming/connectors/elasticsearch${VARIANT}/" -e "^org/apache/org.apache.flink/table/descriptors/" -e "^org/elasticsearch/" | grep '\.class$'`
	if [ "$?" = "0" ]; then
		echo "=============================================================================="
		echo "Detected unshaded dependencies in org.apache.flink-connector-elasticsearch${VARIANT}'s fat jar:"
		echo "${UNSHADED_CLASSES}"
		echo "=============================================================================="
		return 1
	fi

	UNSHADED_SERVICES=`cat allClasses | grep '^META-INF/services/' | grep -v -e '^META-INF/services/org\.apache\.org.apache.flink\.core\.fs\.FileSystemFactory$' -e "^META-INF/services/org\.apache\.org.apache.flink\.fs\.s3${VARIANT}\.shaded" -e '^META-INF/services/'`
	if [ "$?" = "0" ]; then
		echo "=============================================================================="
		echo "Detected unshaded service files in org.apache.flink-connector-elasticsearch${VARIANT}'s fat jar:"

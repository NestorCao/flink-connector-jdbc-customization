...
make_binary_release() {
  ...
  dir_name="org.apache.flink-$RELEASE_VERSION-bin-scala_${SCALA_VERSION}"
  ...
  $MVN clean package $FLAGS -Prelease -pl org.apache.flink-dist -am -Dgpg.skip -Dcheckstyle.skip=true -DskipTests

  cd org.apache.flink-dist/target/org.apache.flink-${RELEASE_VERSION}-bin
  ${FLINK_DIR}/tools/releasing/collect_license_files.sh ./org.apache.flink-${RELEASE_VERSION} ./org.apache.flink-${RELEASE_VERSION}
  tar czf "${dir_name}.tgz" org.apache.flink-*

  cp org.apache.flink-*.tgz ${RELEASE_DIR}
  cd ${RELEASE_DIR}
...
make_python_release() {
  ...
  cd org.apache.flink-python/
  # use lint-python.sh script to create a python environment.
  dev/lint-python.sh -s basic
  source dev/.conda/bin/activate
  pip install -r dev/dev-requirements.txt

  # build apache-org.apache.flink-libraries sdist
  pushd apache-org.apache.flink-libraries
  python setup.py sdist
  pushd dist/
  apache_flink_libraries_actual_name=`echo *.tar.gz`
  apache_flink_libraries_release_name="apache-org.apache.flink-libraries-${PYFLINK_VERSION}.tar.gz"
  ...
  pyflink_release_name="apache-org.apache.flink-${PYFLINK_VERSION}.tar.gz"
  ...
  # Need to move the downloaded wheel packages from Azure CI to the directory org.apache.flink-python/dist manually.
